...
async function searchProduct(value) {
	try {
		const res = await axiosInstance.get(`/product/search?query=${value}`);
		return res;
	} catch (error) {
		return error;
	}
...
async function getProductBySubcatName(subcat_id) {
	try {
		const res = await axiosInstance.get(`/product/subcategory/${subcat_id}`);
		return res;
	} catch (error) {
		return error;
	}
